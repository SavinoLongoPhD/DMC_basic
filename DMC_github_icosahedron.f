...
c
c	Update: June 16th, 2022 - this version includes the confinement into
c	an icosahedral and dodecahedral potential week using the method 
c	of Onaka.
c
...
c	additional variables, de-comment for 2-electrons calculations
...
c	if(c<0)g=1e6	!sigma_u

c	if(a<0)g=1e6	!Pi_u_y

c	if(b<0)g=1e6	!Pi_u_x
...
c
c	Icosahedral confinement, Onaka - like
c
c “al” represents the half edge of the circumscribed cube. A vertex of the
c icosahedron therefore, the one located on the upper face of the cube, is
c located at (al/fi, 0, al) where “fi” is the golden ratio. 1/fi = 0.618
...
c	   print *,j*dr,isto(j)
	...
	function rico(x,y,z,al)



	real rico,G1,G2,g,ga,de,ep,al,he,ze

	ga = 1./sqrt(3.)

c	cde = sqrt((5.-sqrt(5.))/10.) ! questi per dodecaedro
c	cep = sqrt((5.+sqrt(5.))/10.)

c	ze = sqrt((3.-sqrt(5.))/6.) ! parametri fissi icosaedro
c	he = sqrt((3.+sqrt(5.))/6.)
...
c hard icosahedron (with sharp edges): function g used as in Onaka

	rico = 0.5
	if(abs(gg(u,v,w,ga,ga,ga)).gt.1)rico=2
	if(abs(gg(u,v,w,-ga,ga,ga)).gt.1)rico=2
	if(abs(gg(u,v,w,ga,-ga,ga)).gt.1)rico=2
	if(abs(gg(u,v,w,ga,ga,-ga)).gt.1)rico=2
	if(abs(gg(u,v,w,ze,he,0.)).gt.1)rico=2
	if(abs(gg(u,v,w,ze,-he,0.)).gt.1)rico=2
	if(abs(gg(u,v,w,0.,ze,he)).gt.1)rico=2
	if(abs(gg(u,v,w,0.,ze,-he)).gt.1)rico=2
	if(abs(gg(u,v,w,he,0.,ze)).gt.1)rico=2
	if(abs(gg(u,v,w,he,0.,-ze)).gt.1)rico=2

c for smooth Onaka shapes, replace the previous lines with the following
c rico <1: inside, rico >1: outside

c	p = 320		! exponent
c
c	G1 = abs(g(u,v,w,ga,ga,ga))**p+abs(g(u,v,w,-ga,ga,ga))**p+ 
c     ^     	abs(g(u,v,w,ga,-ga,ga))**p+abs(g(u,v,w,ga,ga,-ga))**p

c	G2 = abs(g(u,v,w,ze,he,0.))**p+abs(g(u,v,w,ze,-he,0.))**p+ 
c     ^    	abs(g(u,v,w,0.,ze,he))**p+abs(g(u,v,w,0.,ze,-he))**p+ 
c     ^ 	abs(g(u,v,w,he,0.,ze))**p+abs(g(u,v,w,he,0.,-ze))**p

c	rico = (G1+G2)**(1/p)  !ho eliminato 1/
	return
